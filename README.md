...
- BotFather `/setprivacy` → Disable 讓 bot 收得到群裡的一般訊息。預設下「你＋bot」兩人群組自動回、較多人群組需 @mention（照片的 @mention 在 caption 也偵測得到）。
- **`CTB_GROUP_AUTO_RESPOND=1`**（建議專屬飲食群開啟）：bot 對群裡「每則訊息」都回應、免 @mention——配合多人版 spec「只對食物/體重訊息回應、純閒聊保持安靜」，成員直接丟食物照片就會記。改 `.env` 後需重啟 bot。
